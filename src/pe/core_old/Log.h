...
namespace pe {
  ...
  class LogRuleSet {
    // The ruleset determines what log messages are sent to the PE system log file
  ...
  public:
    ...
    // at level pe::WarningMessage or higher priority.
    LogRuleSet() {
      m_rules.push_back(rule_type(pe::InfoMessage, "console"));
    }
  ...
  /// log_namespace.
  std::ostream& pe_out( int log_level = pe::InfoMessage,
  ...
  void set_debug_level( int log_level ) PE_DEPRECATED;
  ...
  void set_output_stream( std::ostream& stream ) PE_DEPRECATED;

} // namespace pe

#endif // __PE_CORE_LOG_H__
