...
///
#ifndef __PE_MATH_VECTOR_H__
#define __PE_MATH_VECTOR_H__
...
namespace math {
  ...
  public:
    ...
    /// proxy object instead, see pe::VectorProxy.
    ...
    /// Generalized copy constructor, from arbitrary PE vector expressions.
    template <class T>
    Vector( VectorBase<T> const& v ) {
      PE_ASSERT( v.impl().size()==SizeN, ArgumentErr() << "Vector must have dimension " << SizeN << "." );
    ...
    /// Generalized assignment operator, from arbitrary PE vector expressions.
    template <class T>
    ...
    void set_size( unsigned new_size, bool /*preserve*/ = false ) {
      PE_ASSERT( new_size==size(), ArgumentErr() << "Cannot change size of fixed-size Vector." );
    }
  ...
  public:
    ...
    VectorProxy& operator=( VectorProxy const& v ) {
      PE_ASSERT( v.size()==size(), ArgumentErr() << "Vector must have dimension " << size() << " in vector proxy assignment." );
    ...
    VectorProxy& operator=( VectorBase<T> const& v ) {
      PE_ASSERT( v.impl().size()==size(), ArgumentErr() << "Vector must have dimension " << size() << " in vector proxy assignment." );
      Vector<value_type> tmp( v );
      VectorAssignImpl<VectorProxy,Vector<value_type> >::assign(*this,tmp);
      return *this;
    }
    ...
    /// Change the size of the vector. Elements in memory are preserved when specified.
    void set_size( unsigned new_size, bool preserve = false ) {
      PE_ASSERT( new_size==size(), ArgumentErr() << "Cannot resize a vector proxy." );
    }
  ...
  public:
    ...
    VectorTranspose& operator=( VectorTranspose<OtherT> const& v ) {
      PE_ASSERT( v.size()==size(), ArgumentErr() << "Vectors must have same size in transposed vector assignment" );
      m_vector = v.child();
      return *this;
    }
  ...
  public:
    ...
    SubVector& operator=( SubVector const& v ) {
      PE_ASSERT( v.size()==size(), ArgumentErr() << "Vectors must have same size in subvector assignment" );
      Vector<value_type> tmp( v );
    ...
    SubVector& operator=( VectorBase<OtherT> const& v ) {
      PE_ASSERT( v.impl().size()==m_size, ArgumentErr() << "Vectors must have same size in subvector assignment" );
      Vector<value_type> tmp( v );
      VectorAssignImpl<SubVector,Vector<value_type> >::assign(*this,tmp);
      return *this;
    }
  ...
  public:
    ...
    VectorBinaryFunc( Vector1T const& v1, Vector2T const& v2 ) : v1(v1), v2(v2), func() {
      PE_ASSERT( v1.size() == v2.size(), ArgumentErr() << "Vectors must have same size in VectorBinaryFunc" );
    }
  ...
  /// you will have to explicitly request this one as pe::math::equal().
  template <class Vector1T, class Vector2T>
