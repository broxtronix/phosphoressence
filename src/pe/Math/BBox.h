...
#ifndef __PE_MATH_BBOX_H__
#define __PE_MATH_BBOX_H__
...
namespace math {
  ...
  namespace vector_containment_comparison {
    ...
    inline bool operator<( VectorBase<VectorT1> const& v1, VectorBase<VectorT2> const& v2 ) {
      PE_ASSERT( v1.impl().size() == v2.impl().size(), ArgumentErr() << "Cannot compare vectors of different length." );
    ...
    inline VectorT1 max( VectorBase<VectorT1> const& v1, VectorBase<VectorT2> const& v2 ) {
      PE_ASSERT( v1.impl().size() == v2.impl().size(), ArgumentErr() << "Cannot compute max of vectors of different length." );
      VectorT1 v3;
    ...
    inline VectorT1 min( VectorBase<VectorT1> const& v1, VectorBase<VectorT2> const& v2 ) {
      PE_ASSERT( v1.impl().size() == v2.impl().size(), ArgumentErr() << "Cannot compute min of vectors of different length." );
      VectorT1 v3;
      v3.set_size(v1.impl().size());
      for( unsigned i=0; i<v1.impl().size(); ++i)
  ...
  public:
    ...
    void grow( VectorBase<VectorT> const& point ) {
      PE_ASSERT(m_min.size() == 0 || point.impl().size() == m_min.size(), ArgumentErr() << "Vector must have dimension " << m_min.size() << ".");
      if (m_min.size() == 0) {
        m_min = point;
        m_max = point;
      }
    ...
    void crop( BBoxBase<BBoxT1, RealT1, DimN1> const& bbox ) {
      PE_ASSERT(m_min.size() == 0 || bbox.min().size() == m_min.size(), ArgumentErr() << "BBox must have dimension " << m_min.size() << ".");
      for( unsigned i=0; i<m_min.size(); ++i ) {
  ...
  public:
    ...
    /// bounding box.
    RealT width() const {
      PE_ASSERT(this->min().size() >= 1, LogicErr() << "BBox must be of dimension >= 1 to get width.");
      return this->max()[0] - this->min()[0];
    }
    ...
    /// or greater.
    RealT height() const {
      PE_ASSERT(this->min().size() >= 2, LogicErr() << "BBox must be of dimension >= 2 to get height.");
      return this->max()[1] - this->min()[1];
    }
    ...
    /// or greater.
    RealT depth() const {
      PE_ASSERT(this->min().size() >= 3, LogicErr() << "BBox must be of dimension >= 3 to get depth.");
      return this->max()[2] - this->min()[2];
    }
