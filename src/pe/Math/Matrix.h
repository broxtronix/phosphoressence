...
///
#ifndef __PE_MATH_MATRIX_H__
#define __PE_MATH_MATRIX_H__
...
namespace math {
  ...
  struct MatrixBase {
    ...
    void set_identity() {
      PE_ASSERT( impl().rows()==impl().cols(), LogicErr() << "Only square matrices can be identity matrices." );
      int n=impl().rows();
      for( int i=0; i<n; ++i )
        for( int j=0; j<n; ++j )
  ...
  public:
    ...
    /// shallow proxy object instead, see pe::MatrixProxy.
    ...
    /// Generalized copy constructor, from arbitrary PE matrix expressions.
    template <class T>
    Matrix( MatrixBase<T> const& m ) {
      PE_ASSERT( m.impl().rows()==RowsN && m.impl().cols()==ColsN, ArgumentErr() << "Matrix must have dimensions " << RowsN << "x" << ColsN << "." );
    ...
    /// Generalized assignment operator, from arbitrary PE matrix expressions.
    template <class T>
    ...
    /// Temporary-free generalized assignment operator, from arbitrary PE matrix expressions.
    ...
    void set_size( unsigned new_rows, unsigned new_cols, bool /*preserve*/ = false ) {
      PE_ASSERT( new_rows==rows() && new_cols==cols(),
                 ArgumentErr() << "Cannot resize a fixed-size Matrix." );
    }

    /// Access an element
    value_type& operator()( unsigned row, unsigned col ) {
#if defined(PE_ENABLE_BOUNDS_CHECK) && (PE_ENABLE_BOUNDS_CHECK==1)
      PE_ASSERT( row < rows() && col < cols(), LogicErr() << "operator() ran off end of matrix" );
#endif
  ...
  public:
    ...
    MatrixProxy& operator=( MatrixProxy const& m ) { 
      PE_ASSERT( m.rows()==RowsN && m.cols()==ColsN, ArgumentErr() << "Matrix must have dimensions " << RowsN << "x" << ColsN << "." );
  ...
  public:
    ...
    /// Standard copy assignment operator.
    MatrixProxy& operator=( MatrixProxy const& m ) { 
      PE_ASSERT( m.rows()==rows() && m.cols()==cols(), ArgumentErr() << "Matrix must have dimensions " << rows() << "x" << cols() << "." );
      Matrix<ElemT> tmp( m );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }
    ...
    template <class T>
    MatrixProxy& operator=( MatrixBase<T> const& m ) { 
      PE_ASSERT( m.impl().rows()==rows() && m.impl().cols()==cols(), ArgumentErr() << "Matrix must have dimensions " << rows() << "x" << cols() << "." );
      Matrix<ElemT> tmp( m );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }
  ...
  public:
    ...
    MatrixRow& operator=( MatrixRow const& v ) {
      PE_ASSERT( v.size()==size(), ArgumentErr() << "Vectors must have same size in matrix row assignment." );
    ...
    MatrixRow& operator=( VectorBase<OtherT> const& v ) {
      PE_ASSERT( v.impl().size()==size(), ArgumentErr() << "Vectors must have same size in matrix row assignment." );
      Vector<value_type> tmp( v );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }

    /// Temporary-free generalized assignment operator, from arbitrary PE vector expressions.
    /// This is a performance-optimizing function to be used with caution!
    template <class OtherT>
  ...
  public:
    ...
    MatrixCol& operator=( MatrixCol const& v ) {
      PE_ASSERT( v.size()==size(), ArgumentErr() << "Vectors must have same size in matrix column assignment." );
      Vector<value_type> tmp( v );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }
    ...
    MatrixCol& operator=( VectorBase<OtherT> const& v ) {
      PE_ASSERT( v.impl().size()==size(), ArgumentErr() << "Vectors must have same size in matrix column assignment." );
      Vector<value_type> tmp( v );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }
  ...
  public:
    ...
    SubMatrix& operator=( SubMatrix const& m ) { 
      PE_ASSERT( m.rows()==rows() && m.cols()==cols(), ArgumentErr() << "Matrices must have same size in submatrix assignment." );
      Matrix<value_type> tmp( m );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }
    ...
    SubMatrix& operator=( MatrixBase<T> const& m ) { 
      PE_ASSERT( m.impl().rows()==rows() && m.impl().cols()==cols(), ArgumentErr() << "Matrices must have same size in submatrix assignment." );
      Matrix<value_type> tmp( m );
      std::copy( tmp.begin(), tmp.end(), begin() );
      return *this;
    }
  ...
  public:
    ...
    MatrixBinaryFunc( Matrix1T const& m1, Matrix2T const& m2 ) : m1(m1), m2(m2), func() {
      PE_ASSERT( m1.rows() == m2.rows() && m1.cols() == m2.cols(), ArgumentErr() << "Matrices must have same size in MatrixBinaryFunc" );
    }
  ...
  typename MatrixT::value_type det( MatrixBase<MatrixT> const& m ) {
    typename MatrixT::value_type result = typename MatrixT::value_type();
    PE_ASSERT( m.impl().rows() == m.impl().cols(), pe::ArgumentErr() << "Can only compute determinant of a square matrix." );
    ...
    while( !s.empty() ) {
      ...
      s.pop();
      PE_ASSERT( a.rows() == a.cols(), pe::LogicErr() << "Matrix has become non-square." );
      unsigned dim = a.rows();
  ...
  inline Matrix<typename MatrixT::value_type> inverse( MatrixBase<MatrixT> const& m ) {
    ...
    for ( unsigned i=0; i<size; ++i) {
      ...
      if ( buf(i_norm_inf,i) == zero ) 
        pe_throw( MathErr() << "Matrix is singular in inverse()" );
      if ( i_norm_inf != i ) {
        unsigned pbuf = pm(i);
        pm(i) = pm(i_norm_inf);
        pm(i_norm_inf) = pbuf;
