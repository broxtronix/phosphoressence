function Zylot__Visionarie_geiss_aspect_ratio_fix() {
  with (Math) {
    fRating=3;
    gamma=1;
    decay=0.997;
    echo_zoom=0.996629;
    echo_alpha=0;
    echo_orient=1;
    wave_mode=0;
    wave_additive=0;
    wave_usedots=0;
    bWaveThick=1;
    bModWaveAlphaByVolume=0;
    wave_brighten=0;
    wrap=1;
    darken_center=0;
    bRedBlueStereo=0;
    brighten=0;
    darken=1;
    solarize=0;
    invert=0;
    wave_a=0.001;
    fWaveScale=0.01;
    fWaveSmoothing=0.27;
    wave_mystery=-0.38;
    fModWaveAlphaStart=0.71;
    fModWaveAlphaEnd=1.3;
    warp_speed=1;
    warp_scale=1.331;
    zoomexp=1;
    fShader=0;
    zoom=0.999514;
    rot=0;
    cx=0.5;
    cy=0.5;
    dx=0;
    dy=0;
    warp=0.01;
    sx=1;
    sy=1;
    wave_r=1;
    wave_g=1;
    wave_b=1;
    wave_x=0.5;
    wave_y=0.5;
    ob_size=0.5;
    ob_r=0.01;
    ob_g=0;
    ob_b=0;
    ob_a=0;
    ib_size=0.26;
    ib_r=1;
    ib_g=1;
    ib_b=1;
    ib_a=0;
    mv_x=64;
    mv_y=48;
    mv_dx=0;
    mv_dy=0;
    mv_l=0.85;
    mv_r=0.4999;
    mv_g=0.4999;
    mv_b=0.4999;
    mv_a=0;
    wavecode_0_enabled=1;
    wavecode_0_samples=512;
    wavecode_0_sep=0;
    wavecode_0_bSpectrum=0;
    wavecode_0_bUseDots=0;
    wavecode_0_bDrawThick=0;
    wavecode_0_bAdditive=0;
    wavecode_0_scaling=1;
    wavecode_0_smoothing=0.5;
    wavecode_0_r=1;
    wavecode_0_g=1;
    wavecode_0_b=1;
    wavecode_0_a=1;
    wave_0_per_pofloor1=x = .5 + (bass*.2)*sin(sample*2*(time*10));;
    wave_0_per_pofloor2=y = .5 + (bass*.2)*cos(sample*2*(time*10));;
    wave_0_per_pofloor3=r = sample;;
    wave_0_per_pofloor4=g = x;;
    wave_0_per_pofloor5=b = y;;
    wave_0_per_pofloor6=x = (x-0.5)*0.75 + 0.5;  // aspect ratio fix by geiss;
    wavecode_1_enabled=0;
    wavecode_1_samples=512;
    wavecode_1_sep=0;
    wavecode_1_bSpectrum=0;
    wavecode_1_bUseDots=0;
    wavecode_1_bDrawThick=0;
    wavecode_1_bAdditive=0;
    wavecode_1_scaling=1;
    wavecode_1_smoothing=0.5;
    wavecode_1_r=1;
    wavecode_1_g=1;
    wavecode_1_b=1;
    wavecode_1_a=1;
    wavecode_2_enabled=0;
    wavecode_2_samples=512;
    wavecode_2_sep=0;
    wavecode_2_bSpectrum=0;
    wavecode_2_bUseDots=0;
    wavecode_2_bDrawThick=0;
    wavecode_2_bAdditive=0;
    wavecode_2_scaling=1;
    wavecode_2_smoothing=0.5;
    wavecode_2_r=1;
    wavecode_2_g=1;
    wavecode_2_b=1;
    wavecode_2_a=1;
    wavecode_3_enabled=0;
    wavecode_3_samples=512;
    wavecode_3_sep=0;
    wavecode_3_bSpectrum=0;
    wavecode_3_bUseDots=0;
    wavecode_3_bDrawThick=0;
    wavecode_3_bAdditive=0;
    wavecode_3_scaling=1;
    wavecode_3_smoothing=0.5;
    wavecode_3_r=1;
    wavecode_3_g=1;
    wavecode_3_b=1;
    wavecode_3_a=1;
    shapecode_0_enabled=0;
    shapecode_0_sides=3;
    shapecode_0_additive=0;
    shapecode_0_thickOutline=0;
    shapecode_0_textured=0;
    shapecode_0_x=0.5;
    shapecode_0_y=0.5;
    shapecode_0_rad=0.34;
    shapecode_0_ang=0.53;
    shapecode_0_tex_ang=0;
    shapecode_0_tex_zoom=1;
    shapecode_0_r=0.56;
    shapecode_0_g=0.36;
    shapecode_0_b=0;
    shapecode_0_a=1;
    shapecode_0_r2=0.9;
    shapecode_0_g2=1;
    shapecode_0_b2=0;
    shapecode_0_a2=0;
    shapecode_0_border_r=0;
    shapecode_0_border_g=0;
    shapecode_0_border_b=0;
    shapecode_0_border_a=0;
    ang = ang + time;
    shapecode_1_enabled=0;
    shapecode_1_sides=3;
    shapecode_1_additive=0;
    shapecode_1_thickOutline=0;
    shapecode_1_textured=0;
    shapecode_1_x=0.5;
    shapecode_1_y=0.5;
    shapecode_1_rad=0.41;
    shapecode_1_ang=0;
    shapecode_1_tex_ang=0;
    shapecode_1_tex_zoom=1;
    shapecode_1_r=0;
    shapecode_1_g=0.8;
    shapecode_1_b=0.7;
    shapecode_1_a=1;
    shapecode_1_r2=0;
    shapecode_1_g2=0.4;
    shapecode_1_b2=1;
    shapecode_1_a2=0;
    shapecode_1_border_r=1;
    shapecode_1_border_g=1;
    shapecode_1_border_b=1;
    shapecode_1_border_a=0;
    ang = ang - time*.333;
    shapecode_2_enabled=0;
    shapecode_2_sides=3;
    shapecode_2_additive=0;
    shapecode_2_thickOutline=0;
    shapecode_2_textured=0;
    shapecode_2_x=0.5;
    shapecode_2_y=0.5;
    shapecode_2_rad=0.21;
    shapecode_2_ang=0;
    shapecode_2_tex_ang=0;
    shapecode_2_tex_zoom=1;
    shapecode_2_r=0.84;
    shapecode_2_g=0;
    shapecode_2_b=0;
    shapecode_2_a=1;
    shapecode_2_r2=1;
    shapecode_2_g2=0.58;
    shapecode_2_b2=0;
    shapecode_2_a2=0;
    shapecode_2_border_r=1;
    shapecode_2_border_g=1;
    shapecode_2_border_b=1;
    shapecode_2_border_a=0;
    ang = ang + time*4;
    shapecode_3_enabled=0;
    shapecode_3_sides=4;
    shapecode_3_additive=0;
    shapecode_3_thickOutline=0;
    shapecode_3_textured=0;
    shapecode_3_x=0.5;
    shapecode_3_y=0.5;
    shapecode_3_rad=0.1;
    shapecode_3_ang=0;
    shapecode_3_tex_ang=0;
    shapecode_3_tex_zoom=1;
    shapecode_3_r=1;
    shapecode_3_g=0;
    shapecode_3_b=0;
    shapecode_3_a=1;
    shapecode_3_r2=0;
    shapecode_3_g2=1;
    shapecode_3_b2=0;
    shapecode_3_a2=0;
    shapecode_3_border_r=1;
    shapecode_3_border_g=1;
    shapecode_3_border_b=1;
    shapecode_3_border_a=0.1;
    warp = 0;
    decay = .92;
    // per_pixel_1=zoom = zoom + .1;
    // per_pixel_2=rot = rot + rad*.1;
  }
}
register_preset( Zylot__Visionarie_geiss_aspect_ratio_fix);
