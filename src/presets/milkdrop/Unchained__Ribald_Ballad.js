function Unchained__Ribald_Ballad() {
  with (Math) {
    fRating=3;
    gamma=1;
    decay=0.991;
    ...
    pulse=if_milk(above(abs(pulse),20),-20,pulse+(bass_thresh+mid+thresh+treb_thresh)*.035-(bass+treb+mid)*.01);
    q1=mid_residual;
    q2=bass_residual;
    q3=treb_residual;
    q4=sin(pulse);
    ...
    // per_pixel_1=rot=if_milk(above(q4,0),rad*.2*q5,rot+.3*sin(ang*3.14*(q1+q2+q3)));
    // per_pixel_2=zoom=if_milk(above(q2,0),zoom-cos(rad*3.14*q2)*.1,if_milk(above(q3,0),1+q1*.05,1+.07*cos(ang*10*q1)));
  }
}
register_preset( Unchained__Ribald_Ballad);
