function Unchained__Unclaimed_Wreckage_2_Shamanic() {
  with (Math) {
    fRating=3;
    gamma=2.05;
    decay=0.92;
    ...
    // per_pixel_5=*cos(rad+3.14*if_milk(above(grid,0),snur,bnot(snur))*(.5+.5*sin(rad*3.14*q1)))*q4;
    // per_pixel_6=zoom=zoom+.003*sin(rad*2+rad*q2)*sign(snee)-.01*sin(rad*2*q3);
    // per_pixel_7=rot=if_milk(rot,rot*sign(snur),q6*sign(-snur));
    // per_pixel_8=cx=if_milk(below(x,.5)*above(y,.5),.5+.2*q1,.5);
    // per_pixel_9=cy=if_milk(below(x,.5)*below(y,.5),.5+.2*q3,.5);
    // per_pixel_10=sx=sx+.17*sin(rad)*q1*grid*(1-abs(rot));
    // per_pixel_11=sy=sy+.17*sin(rad)*q2*grid*(1-abs(rot));
  }
}
register_preset( Unchained__Unclaimed_Wreckage_2_Shamanic);
