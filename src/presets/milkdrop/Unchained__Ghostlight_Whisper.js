function Unchained__Ghostlight_Whisper() {
  with (Math) {
    fRating=3;
    gamma=1;
    decay=0.982;
    ...
    pulse=if_milk(above(abs(pulse),20),-20,pulse+(bass_thresh+mid_thresh+treb_thresh)*.01);
    q1=mid_residual;
    q2=bass_residual;
    q3=treb_residual;
    q4=sin(pulse);
    ...
    mv_y=q6*q7;
    zoom=zoom+.01*q1;
    // per_pixel_1=gridx=bnot((q7*sin(x*3.14))%2);
    // per_pixel_2=gridy=bnot((q7*sin(y*3.14))%2);
    // per_pixel_3=dx=sin((y-0.5)*q1*6.2)*.01+q5*sin((y-0.5)*q2*6.2)*.01;
    // per_pixel_4=dy=cos((x-0.5)*q2*6.2)*.01+q4*cos((x-0.5)*q1*6.2)*.01;
    // per_pixel_5=grid=sin(sigmoid(sin(y*6.28*q2),sin(x*6.28*q5))*(10+q7));
    // per_pixel_6=rot=rot*sign(grid)*q4;
    // per_pixel_7=sx=sx+grid*.03;
    entropy=2;
  }
}
register_preset( Unchained__Ghostlight_Whisper);
