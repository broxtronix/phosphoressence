function Rovastar__Harlequins_Fractal_Encounter_2() {
  with (Math) {
    fRating=4;
    gamma=1;
    decay=1;
    ...
    q6 = beat;
    wave_mode = if_milk(beateven,7,0);
    mv_a = if_milk(beat,if_milk(equal(beatcounter,6),1,0),0);
    mv_r = if_milk(mv_a,random_integer(2),0);
    mv_b = if_milk(mv_a,random_integer(2),0);
    mv_g= if_milk(mv_a,random_integer(2),0);
    // per_pixel_1=box =0.5+0.8*(2*x%4+2*y%2);
    // per_pixel_2=q1 = 8.45+0.3*(sin(pow(x,3)+0.177*time)-cos(pow(y,3)+0.223*time));
    // per_pixel_3=q7 = above(box,1);
    // per_pixel_4=zoom = if_milk(q7,(q1*.1) + q6*6 ,zoom);
    // per_pixel_5=rot = if_milk(q7,0.63*sin(0.385*time + 0.12*sin(0.67*time) + 0.1*q4 + 0.12*q2 +q6*50),rot);
    // per_pixel_6=cx = cx - 0.15*sin(2*q4);
    // per_pixel_7=cy = cy + 0.14*sin(2*q2);
    // per_pixel_8=sx = if_milk(q7,sx+q6*18,sx);
    // per_pixel_9=sy = if_milk(q7,sy+q6*18,sy);
    beatcounter =0;
  }
}
register_preset( Rovastar__Harlequins_Fractal_Encounter_2);
