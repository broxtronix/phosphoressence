function Unchained__Unclaimed_Wreckage() {
  with (Math) {
    fRating=3;
    gamma=1;
    decay=0.999;
    ...
    // per_pixel_6=zoom=zoom+.03*sin(rad*2+rad*q2)*sign(snee)-.06*sin(rad*2*q3);
    // per_pixel_7=rot=if_milk(rot,rot*sign(snur),q6*sign(-snur));
    // per_pixel_8=cx=if_milk(below(x,.5)*below(y,.5),.5+.2*q1,.5);
    // per_pixel_9=cy=if_milk(below(x,.5)*below(y,.5),.5+.2*q3,.5);
    // per_pixel_10=sx=sx+.17*sin(rad)*q1*grid;
    // per_pixel_11=sy=sy+.17*sin(rad)*q2*grid;
  }
}
register_preset( Unchained__Unclaimed_Wreckage);
