function Rovastar__Harlequins_Dynamic_Fractal_2() {
  with (Math) {
    fRating=3;
    gamma=1;
    decay=1;
    ...
    ob_size = 0.005+ 0.02*abs(sin(time*0.7834));
    ib_r = 0.8*(1- ob_r)+0.2;
    ib_g = 0.7*(1- ob_g)+0.3;
    ib_b = ib_b - 0.5*sin(time*1.731);
    ...
    dy =0.1*ypos;
    q4 = ypos;
    // per_pixel_1=box =0.12+2*x%2+2*y%2;
    // per_pixel_2=q1 = 6.4+1.2*(sin(x+(0.6*time))-cos(y+(0.432*time)));
    ...
    // per_pixel_5=dx = if_milk(above(box,1),q4*3,dx);
    // per_pixel_6=dy= if_milk(above(box,1),q2*3,dy);
  }
}
register_preset( Rovastar__Harlequins_Dynamic_Fractal_2);
