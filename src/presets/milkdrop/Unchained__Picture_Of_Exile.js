function Unchained__Picture_Of_Exile() {
  with (Math) {
    fRating=2;
    gamma=1;
    decay=0.996;
    ...
    // per_pixel_5=sx=sx-if_milk(above(q7,3),bnot(grid)*q3*.2,.1*q2*bnot(q8%5));
    // per_pixel_6=sy=sy-if_milk(below(q7,3),bnot(grid)*q2*.2,.1*q3*bnot(q8%2));
    // per_pixel_7=rot=if_milk(equal(grid,3),q3,bnot(grid%q7)*above(x,.5)*above(y,.5)*sin(zoom));
  }
}
register_preset( Unchained__Picture_Of_Exile);
