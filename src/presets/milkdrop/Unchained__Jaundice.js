function Unchained__Jaundice() {
  with (Math) {
    ...
    ob_g=wave_g;
    ob_b=wave_b;
    // per_pixel_1=grid=x*100%(above(q1,q4)+above(q2,q5)+above(q3,q4)) + y*100%(above(q1,q5)+above(q2,q4)+above(q3,q5));
    // per_pixel_2=rot=if_milk(grid,.07*cos(rad*3.14+x*q1*3.14+y*q2*3.14)*(q5+q4),0);
    // per_pixel_3=zoom=zoom-bnot(grid)*sin(x*q3*3.14+y*q5*3.14)*.07;
  }
}
register_preset( Unchained__Jaundice);
