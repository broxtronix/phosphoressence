function Unchained_and_Che__Oddnezz_4_Done_it_again() {
  with (Math) {
    fRating=2;
    gamma=2;
    decay=0.9;
    ...
    // per_pixel_1=grid=pow(sin(rad*6*sin(q2)+x*y*16*q1)*2,1+q2%5)%2;
    // per_pixel_2=snee=.5*sin(q2*q1);
    // per_pixel_3=rot=bnot(grid)*above(x,.5+snee)*above(y,.5-snee);
    // per_pixel_4=sx=sx+sin((q2+.5)*x)*bnot(rot)*.003;
    // per_pixel_5=sy=sy+sin((q2+3.4)*y)*bnot(rot)*.003;
  }
}
register_preset( Unchained_and_Che__Oddnezz_4_Done_it_again);
