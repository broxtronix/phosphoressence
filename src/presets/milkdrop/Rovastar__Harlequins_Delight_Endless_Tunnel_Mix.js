function Rovastar__Harlequins_Delight_Endless_Tunnel_Mix() {
  with (Math) {
    fRating=3;
    gamma=1;
    decay=1;
    ...
    oldq3 = q3;
    my_ib_size = 0.01+abs(0.01*sin(0.1*random_integer(1000)));
    ib_size = if_milk(beat,my_ib_size,old_ib_size);
    old_ib_size = ib_size;
    ib_r = ib_r + 0.5*sin(time*3.934);
    ...
    // per_pixel_3=zoom = if_milk(above(box,1),q1*.1,zoom*0.95);
    // per_pixel_4=rot = if_milk(above(box,1),sin(0.885*time),0);
    // per_pixel_5=dx = if_milk(above(box,1),sin(0.542*time),0);
    // per_pixel_6=dy= if_milk(above(box,1),sin(0.581*time),0);
  }
}
register_preset( Rovastar__Harlequins_Delight_Endless_Tunnel_Mix);
