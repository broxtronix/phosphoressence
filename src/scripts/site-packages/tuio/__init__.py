...
class Tracking(object):
    ...
    def load_profiles(self):
		...
		print "TUIO adding callback for ",profile.address
    ...
    def update(self):
        ...
        """
        try:
            # Drain the incoming message buffer.
            while True:
                (data,source) = self.socket.recvfrom(512)
                if (len(source) == 0):
                    break
                sourcestring = source.__str__()
                self.manager.handle(data,sourcestring)
        except socket.error:
            pass

    def callback(self, source, *incoming):
        """
        Gets called by the CallbackManager if a new message was received 
        """
        # print "Tracking.callback!! source=",source," incoming=",incoming
        message = incoming[0]
        if message:
            ...
            if profile is not None:
                try:
                    # print "CALLING TUIO user callback!!!\n"
                    getattr(profile, command)(self, source, message)
                except AttributeError:
                    pass
