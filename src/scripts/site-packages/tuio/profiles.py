...
class TuioProfile(object):
    ...
    def set(self, client, source, message):
        """
    ...
    def alive(self, client, source, message):
        """
    ...
    def fseq(self, client, source, message):
        """
    ...
    def objs(self):
        ...
        for obj in self.objects.itervalues():
            for source in self.sessions:
                if obj.sessionid in self.sessions[source]:
                    yield obj
...
class Tuio2DcurProfile(TuioProfile):
    ...
    def set(self, client, source, message):
        ...
        if sessionid not in self.objects:
            self.objects[sessionid] = Tuio2DCursor(sessionid,source)
        self.objects[sessionid].update(sessionid, message[4:])

    def alive(self, client, source, message):
        if client.refreshed():
            self.sessions[source] = message[3:]
            for obj in self.objects.keys():
                if obj not in self.sessions[source]:
